...
IntronFeatures <- read.table("../output/IntronFeatures.txt.gz", sep='\t', header=T) %>%
  ...
  mutate(junc_id=paste(paste0("chr", chrom), start, newStop, sep=":")) %>%
  dplyr::select(-c("chrom", "start", "stop", "newStop"))
...
IntronFeaturesAndEffectSizes <- leafcutter.merged.w.features %>%
  dplyr::select(junc_id, logef, DonorScore, AcceptorScore, ss_dist, bp_scr, ppt_len, ppt_scr, svm_scr) %>%
